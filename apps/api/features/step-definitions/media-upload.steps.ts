...
function parseSize(spec: string): number {
  ...
  assert.ok(match, `unrecognized size spec "${spec}" -- expected e.g. "10KB" or "3MB"`);
...
Then(
  ...
  async function (this: AuthWorld) {
    ...
    const streamResponse = await this.http.get(`/api/media/${this.uploadedMediaId}`);
...
When('the uploaded media is streamed back', async function (this: AuthWorld) {
  assert.ok(this.uploadedMediaId, 'expected a prior successful upload step');
  this.mediaResponse = await this.http.get(`/api/media/${this.uploadedMediaId}`);
});
...
When(
  ...
  async function (this: AuthWorld) {
    assert.ok(this.uploadedMediaId, 'expected a prior successful upload step');
    const first = await this.http.get(`/api/media/${this.uploadedMediaId}`);
